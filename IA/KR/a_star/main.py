...
class Problema:
	...
	# defineste datele problemei

	def __init__(self):
		#lista de noduri; un nod are informatie si cost aproximativ, a fiind nod de pornire are euristica infinit
		self.noduri = [
...
class NodParcurgere:
	...
	def __init__(self, nod_graf, parinte=None, g=0, f=None):
		...
		# fata de clasa nod simplu, clasa nod_parcurgere are si un parinte si cost
		if f is None :
			self.f = self.g + self.nod_graf.h
			# functia euristica ( costul pana la nodul curent, g + costul minim h )
		else:
			self.f = f
        ...
        for (nod_succesor, cost_succesor) in l_succesori:
            ...
            if (not nod_curent.contine_in_drum(nod_succesor)):
                ...
                if nod_parcg_vechi is not None:	# "nod_succesor" e in closed
					...
					# verific daca este in open pentru ca se poate ca unii din succesorii precedenti adaugati in closed
					# sa aiba o ruta diferita din nodul parinte pana la acesta, dar cu un drum de cost mai mic.
