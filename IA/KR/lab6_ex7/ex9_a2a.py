...
class InfoJoc:
    ...
    def final(self, move):
        rez = elem_identice(self.matr[0]) or elem_identice(self.matr[1]) or elem_identice(self.matr[2]) \
    ...
    def estimeaza_scor(self, stare, move):
        fin = self.final(move)
        if move == 1:
            ...
            return 0
        #pct c)
        # pt 2 posibilitati de castig:
        # muta, apoi verifica daca scorul din self.has_winning_position > 2
        # daca da, atunci returneaza 99 (respectiv -99), daca nu, nu.
        # deoarece, daca are o posibilitate de a castiga si nu este randul sau,
        # acesta va fi blocat, iar daca este randul sau, este tratat la punctul
        # b, chiar codul de mai jos unde verifica daca > 0.
        """
        if self.has_winning_position(stare.j_curent) == 2:
        ...
        if self.has_winning_position(stare.j_curent)>0:
            ...
            return -self.linii_deschise(InfoJoc.SIMBOL)
...
class Stare:
    ...
    def mutari(self):
        ...
        l_stari_mutari = [Stare(mutare, InfoJoc.SIMBOL, self.adancime - 1, parinte=self) for mutare in l_mutari]
...
def min_max(stare, move):
    if stare.tabla_joc.final(move) or stare.adancime == 0:
...
def afis_daca_final(stare_curenta, move):
    final = stare_curenta.tabla_joc.final(move)
    if (final):
        if move == 1:
            print("A castigat AI")
        else:
            print("A castigat UTILIZATOR")
...
def main():
    ...
    raspuns_valid = False
    while not raspuns_valid:
        move = input("Incepi tu sau calculatorul?\n 1. EU\n 2.AI\n  ").lower()
        if (move in ['1', '2']):
            raspuns_valid = True
        else:
    ...
    while True:
                    ...
                    for linie in range(InfoJoc.NR_COLOANE):
                        for coloana in range(InfoJoc.NR_COLOANE):
                            ...
                            if InfoJoc.celuleGrid[linie][coloana].collidepoint(
                                ...
                                if stare_curenta.tabla_joc.matr[linie][coloana] == InfoJoc.SIMBOL:
                                    ...
                                    stare_curenta.tabla_joc.matr[linie][coloana] = InfoJoc.SIMBOL
                                    stare_curenta.tabla_joc.deseneaza_grid()
                                    ...
                                    if (afis_daca_final(stare_curenta, move)):
                                        break
        ...
        # --------------------------------
            ...
            if tip_algoritm == '1':
                stare_actualizata = min_max(stare_curenta, move)
            else:  # tip_algoritm==2
