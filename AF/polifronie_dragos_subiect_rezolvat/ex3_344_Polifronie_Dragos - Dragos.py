...
def citire(nume_fisier):
    g = open(nume_fisier)
    n, m = [int(x) for x in g.readline().split()]
   # 
   # la final
    #m = int(g.readline())
    ...
    grad_ext = [0 for i in range(n+1)]
    for i in range(1,n+1):
        
        lista = [int(x) for x in g.readline().split()]
        print(lista)
        grad_ext[i] = lista[0]
        if grad_ext[i] == 0:
            continue
        for k in range(1, 2*grad_ext[i]-1, 2):
            la[i].append([lista[k], lista[k+1], 0])
...
n,m,la, s,t, intra, iese = citire("retea2.in");
ok = 1
for i in range(1,n+1):
    if i != s and i != t:
        if intra[i] != iese[i]:
